...
n_step = 3
n_hidden = 32 #hidden units이 클수록 복잡한 연산 가능
...
for i in range(0, len(test_X) - n_step + 1):
    _test_X = test_X[i:i + n_step]
    _test_Y = test_Y[i + n_step - 1]
...
for _ in range(num_layers):
    cell = tf.nn.rnn_cell.GRUCell(n_hidden) #n_hidden은 output에서의 unit size와 관련
    cells.append(cell)
